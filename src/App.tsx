...
import {
  ...
  Star, Flame, Shield, Swords, Sun, Moon, LogOut,
} from 'lucide-react';

// ─── Data ────────────────────────────────────────────────────────────────────

const mockMatches = [
  { id: 1,  home: 'FLA', away: 'VAS', homeName: 'Flamengo',     awayName: 'Vasco'         },
  { id: 2,  home: 'PAL', away: 'COR', homeName: 'Palmeiras',    awayName: 'Corinthians'   },
  { id: 3,  home: 'SAO', away: 'SAN', homeName: 'São Paulo',    awayName: 'Santos'        },
  { id: 4,  home: 'CAM', away: 'CRU', homeName: 'Atlético-MG',  awayName: 'Cruzeiro'     },
  { id: 5,  home: 'GRE', away: 'INT', homeName: 'Grêmio',       awayName: 'Internacional' },
  { id: 6,  home: 'FLU', away: 'BOT', homeName: 'Fluminense',   awayName: 'Botafogo'     },
  { id: 7,  home: 'BAH', away: 'VIT', homeName: 'Bahia',        awayName: 'Vitória'      },
  { id: 8,  home: 'CAP', away: 'CFC', homeName: 'Athletico-PR', awayName: 'Coritiba'     },
  { id: 9,  home: 'FOR', away: 'CEA', homeName: 'Fortaleza',    awayName: 'Ceará'        },
  { id: 10, home: 'GOI', away: 'ACG', homeName: 'Goiás',        awayName: 'Atl-GO'       },
];
...
function Login({ onLogin, isDark, toggleTheme }: { onLogin: () => void; isDark: boolean; toggleTheme: () => void }) {
  ...
  const inputStyle = (focused: boolean) => ({
    ...
    border: `1.5px solid ${focused ? '#FBBF24' : T.inputBdr(d)}`,
    color: T.text(d),
  });

  return (
    <div
      className="min-h-[100dvh] flex flex-col items-center justify-center px-6 relative overflow-hidden transition-colors duration-300"
        ...
        <div className="text-center space-y-4">
          <div className="mx-auto w-20 h-20 rounded-[20px] flex items-center justify-center"
            style={{
              ...
              boxShadow: '0 0 40px rgba(251,191,36,0.35), 0 8px 24px rgba(0,0,0,0.3)',
            }}>
            ...
            {[
              ...
              <div key={placeholder} className="relative">
                ...
                <input
                  ...
                  style={{ ...inputStyle(false), caretColor: '#FBBF24' }}
...
function Apostar({ isDark }: { isDark: boolean }) {
  const d = isDark;
  type ScoreMap = Record<number, { home: string; away: string }>;
  const [scores, setScores] = useState<ScoreMap>(
    Object.fromEntries(mockMatches.map(m => [m.id, { home: '', away: '' }]))
  );
  const [submitted, setSubmitted] = useState(false);

  const setScore = (id: number, side: 'home' | 'away', val: string) => {
    setScores(s => ({ ...s, [id]: { ...s[id], [side]: val.replace(/\D/g, '').slice(0, 2) } }));
  };

  const filledCount = Object.values(scores).filter((s: { home: string; away: string }) => s.home !== '' && s.away !== '').length;

  return (
    <div className="pb-4 space-y-3">
      {/* Progress */}
      <div className="rounded-xl p-4 mb-2 flex items-center justify-between border"
        style={{ background: d ? 'rgba(251,191,36,0.07)' : 'rgba(251,191,36,0.06)', borderColor: d ? 'rgba(251,191,36,0.15)' : 'rgba(251,191,36,0.2)' }}>
        <div>
          <p className="font-bold text-sm" style={{ color: T.text(d) }}>{filledCount} de {mockMatches.length} palpites</p>
          <p className="text-xs mt-0.5" style={{ color: T.textMuted(d) }}>Preencha todos para enviar</p>
        </div>
        <div className="relative w-12 h-12">
          <svg className="w-12 h-12 -rotate-90" viewBox="0 0 44 44">
            ...
            <circle cx="22" cy="22" r="18" fill="none" stroke="#FBBF24" strokeWidth="4"
              strokeDasharray={`${(filledCount / mockMatches.length) * 113} 113`} strokeLinecap="round" />
          </svg>
          <span className="absolute inset-0 flex items-center justify-center font-black text-xs" style={{ color: T.text(d) }}>
            {Math.round((filledCount / mockMatches.length) * 100)}%
          </span>
        </div>
      </div>
      ...
      {mockMatches.map((match, idx) => {
        ...
        return (
          <motion.div key={match.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.04 }}
            className="rounded-2xl overflow-hidden border" style={{ background: T.surface(d), borderColor: T.border(d) }}>
            <div className="px-4 pt-3 pb-1 flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: T.textMuted(d) }}>Rodada 12</span>
            ...
            <div className="flex items-center px-4 pb-4 pt-2 gap-2">
              ...
              <div className="flex-1 flex flex-col items-center gap-2">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm"
                  style={{ background: T.avatarBg(d), border: `1px solid ${T.border(d)}`, color: T.text(d) }}>
                  {match.home}
                </div>
              ...
              <div className="flex items-center gap-2">
                {(['home', 'away'] as const).map((side, i) => {
                  const val = side === 'home' ? sh : sa;
                  const filled = val !== '';
                  return (
                    <React.Fragment key={side}>
                      <input
                        type="number" inputMode="numeric" value={val} placeholder="0"
                        onChange={e => setScore(match.id, side, e.target.value)}
                        className="w-12 h-12 rounded-xl text-center text-xl font-black outline-none transition-all placeholder:text-slate-600"
                        style={{
                          background: filled ? 'rgba(251,191,36,0.12)' : T.inputBg(d),
                          border: `1.5px solid ${filled ? 'rgba(251,191,36,0.4)' : T.inputBdr(d)}`,
                          color: T.text(d),
                        }}
                      />
                      {i === 0 && (
      ...
      <div className="pt-2">
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            <button key="btn"
              onClick={() => filledCount === mockMatches.length && setSubmitted(true)}
              className="w-full py-4 rounded-xl font-black text-sm tracking-wide transition-all active:scale-95"
              style={{
                background: filledCount === mockMatches.length ? 'linear-gradient(135deg,#FBBF24 0%,#F59E0B 100%)' : T.elevated(d),
                color: filledCount === mockMatches.length ? '#0C1120' : T.textMuted(d),
                border: filledCount === mockMatches.length ? 'none' : `1px solid ${T.border(d)}`,
                boxShadow: filledCount === mockMatches.length ? '0 4px 24px rgba(251,191,36,0.25)' : 'none',
                cursor: filledCount === mockMatches.length ? 'pointer' : 'not-allowed',
              }}
            >
              {filledCount === mockMatches.length ? 'Enviar Palpites' : `Faltam ${mockMatches.length - filledCount} palpites`}
            </button>
          )}
        </AnimatePresence>
      </div>
...
function Ranking({ isDark }: { isDark: boolean }) {
  ...
  return (
    <div className="pb-4 space-y-4">
      ...
      {/* Rules */}
      <div className="rounded-2xl p-5 space-y-3 mt-2 border"
        style={{ background: T.surface(d), borderColor: T.border(d) }}>
...
const headerContent: Record<Tab, { title: string; sub: string }> = {
  apostar: { title: 'Rodada 12',     sub: 'Faça seus palpites'    },
...
export default function App() {
  ...
  const { title, sub } = headerContent[tab];
